...
<template>
  <main
    ...
    aria-label="stanza"
  >
    <div class="home__sky" aria-hidden="true"></div>
    <div class="home__beams" aria-hidden="true"></div>
    <div class="home__floor" aria-hidden="true"></div>
    <div class="home__moonbeam" aria-hidden="true"></div>
    <div class="home__grain" aria-hidden="true"></div>
    ...
    <div class="home__room">
      ...
      >
        ...
        <ul class="home__pegs">
          <li v-for="p in rope" :key="p.slug" class="home__peg-item">
            <button
            ...
            >
              <span class="home__peg" aria-hidden="true">
                <svg viewBox="0 0 32 36" aria-hidden="true" focusable="false">
                  ...
                  <rect x="6" y="2" width="20" height="22" rx="2" fill="url(#peg-grad)" />
                  <rect x="9" y="20" width="14" height="14" rx="1.5" fill="url(#peg-grad)" />
                  <line x1="16" y1="6" x2="16" y2="22" stroke="rgba(0,0,0,0.4)" stroke-width="1" />
                  <circle cx="16" cy="14" r="1.5" fill="#1a140c" />
                </svg>
              </span>
    ...
    >
      ...
      <!-- outer halo via filter, only visible when lit -->
...
<style scoped>
.home {
  --light-radius: clamp(140px, 22vw, 240px);
  --light-soft: clamp(180px, 28vw, 320px);
  --darkness-floor: 0.94;
  --darkness-floor-touch: 0.97;
  --polaroid-w: clamp(100px, 13vw, 168px);
  --polaroid-h: calc(var(--polaroid-w) * 1.22);
  --polaroid-photo-h: calc(var(--polaroid-w) * 0.92);
  --rope-gap-mobile: clamp(16vw, 22vw, 28vw);
  --rope-gap-desktop: clamp(13vh, 16vh, 20vh);
  --grain-opacity: 0.05;
  --moonbeam-opacity: 0.07;
  --dust-opacity: 0.55;
...
.home__sky {
  ...
  inset: 0;
  background:
    /* faint wallpaper texture (very subtle vertical stripes) */
    ...
    /* a window-shaped patch of "moon-lit wall" upper-right — pure CSS, no asset */
      radial-gradient(
...
/* the actual window — a faint cross of mullions in the upper-right "patch" */
.home__sky::before {
  content: '';
  position: absolute;
  width: 14%;
  ...
  background:
    /* mullions */
    linear-gradient(to right, transparent 49.6%, rgba(0, 0, 0, 0.5) 49.6%, rgba(0, 0, 0, 0.5) 50.4%, transparent 50.4%),
    linear-gradient(to bottom, transparent 49.6%, rgba(0, 0, 0, 0.5) 49.6%, rgba(0, 0, 0, 0.5) 50.4%, transparent 50.4%),
    /* moonlit pane */
...
/* a tiny round moon visible inside the window pane */
.home__sky::after {
  content: '';
  position: absolute;
  width: 28px;
...
.home__beams {
  ...
  background:
    ...
    linear-gradient(
      to bottom,
      transparent 30%,
      rgba(0, 0, 0, 0.55) 30.4%,
      rgba(0, 0, 0, 0.55) 30.9%,
      transparent 31.3%
    ),
    linear-gradient(
      to bottom,
      transparent 60%,
      rgba(0, 0, 0, 0.55) 60.4%,
      rgba(0, 0, 0, 0.55) 60.9%,
      transparent 61.3%
    ),
    repeating-linear-gradient(
      90deg,
...
.home__candle-echo {
  position: absolute;
  z-index: 5;
  width: clamp(28px, 4vw + 14px, 48px);
  height: auto;
  bottom: clamp(var(--sp-md), 5vh, var(--sp-xl));
...
.home__room {
  ...
  align-items: stretch;
  min-height: 100dvh;
  padding: clamp(60px, 10vh, 120px) clamp(var(--sp-md), 4vw, var(--sp-2xl))
    clamp(60px, 10vh, 100px);
  gap: var(--rope-gap-desktop);
  pointer-events: auto;
}
@media (max-width: 767px) {
  .home__room {
    gap: var(--rope-gap-mobile);
  }
}
...
.home__polaroid {
  ...
  transition:
    transform 280ms cubic-bezier(0.16, 1, 0.3, 1) var(--lift-delay, 0ms),
    filter 280ms ease-out var(--lift-delay, 0ms);
}
...
.home__caption-title {
  display: block;
  font:
    400 clamp(10px, 0.9vw + 6px, 13px) / 1.25 'Cormorant Garamond',
    serif;
...
.home__caption-date {
  ...
  margin-top: 2px;
  font:
    400 clamp(8px, 0.6vw + 5px, 10px) / 1.2 'Cormorant Garamond',
    serif;
  font-style: italic;
  opacity: 0.6;
}
...
.home::after {
  ...
  background: radial-gradient(
    ...
    rgba(var(--candle-warmth), calc(0.1 * var(--lit, 0))) 0%,
...
.home__cursor-candle {
  ...
  left: var(--mx, 50%);
  top: var(--my, 50%);
  /* anchor at the wick (slightly above center): translate so the flame is at cursor */
...
/* flame group is hidden until lit (mobile: lit on touch; desktop: lit on enter) */
...
@keyframes home-flame-flicker {
  ...
  25% { transform: scale(1.04, 0.97) translateY(-0.4px); }
  50% { transform: scale(0.96, 1.05) translateY(0.3px); }
